#!/usr/bin/env python3
r"""
Next PS-drawn educational scene, introducing recurrence in the complex plane
...
Controls:
    ...
    - / =           Encoder 2: change displayed iteration/trail length
...
SCRIPT_VERSION = "scene2_complex_plane_2026_06_10_streamlined"
...
INTRO_SLIDES = [
    IntroSlide(
        title="From a line to a plane",
        groups=(
            "A real number can only move left or right.",
            "A complex number has two coordinates.",
            "So each value can move around a whole plane.",
        ),
        formula="z = a + bi",
    ),
    IntroSlide(
        title="Use the same repeat idea",
        groups=(
            "Start at the centre of the plane.",
            "Choose a point called c.",
            "Repeat one rule and watch the orbit it creates.",
        ),
        formula="z → z² + c",
    ),
    IntroSlide(
        title="Now guide the point",
        groups=(
            "Move c around the plane with the joystick.",
            "Press play to step through the orbit.",
            "The trail shows where repeated squaring sends z.",
            "The distance |z| tells how far the orbit has moved.",
        ),
        formula=None,
    ),
]
...
class Scene2State:
    ...
    palette_index: int = 0
    status: str = "Move c, then press play."
...
def recurrence_values(state: Scene2State) -> tuple[list[complex], bool]:
    """Return z_0, z_1, ... until max_iter or a large safety threshold."""
...
def visible_values(state: Scene2State) -> tuple[list[complex], bool]:
    values, diverged = recurrence_values(state)
    n = min(state.current_iter + 1, len(values))
    return values[:n], diverged and n == len(values)


def classify_orbit(values: Sequence[complex], diverged: bool) -> str:
    if diverged:
        return "The orbit has run far away from the centre."
    if len(values) <= 1:
        return "The orbit starts at 0. Press play to step through it."

    z = values[-1]
    mag = abs(z)
    if mag < 0.15 and len(values) > 5:
        return "The orbit is being pulled back near the centre."
    if mag < 2.0:
        return "Bounded so far: the trail is staying close."
    if mag < 10.0:
        return "The orbit is stretching outward."
    return "The orbit is moving away very quickly."


def fade_complete(state: Scene2State, now: float, fade_s: float) -> bool:
    return now - state.intro_fade_start >= fade_s


def current_intro_slide(state: Scene2State) -> IntroSlide:
...
def enter_interactive(state: Scene2State) -> None:
    state.phase = AppPhase.INTERACTIVE
    state.playing = False
    state.current_iter = 0
    state.status = "Move c, then press play."
    state.dirty = True


def handle_intro_event(state: Scene2State, event: Event, now: float, fade_s: float) -> bool:
...
def reset_orbit_after_c_move(state: Scene2State, message: str) -> None:
    state.current_iter = 0
    state.playing = False
...
def handle_interactive_event(state: Scene2State, event: Event) -> bool:
    if event.kind == EventKind.SCENE_ACTION:
    ...
    if event.kind == EventKind.NEXT:
        state.status = "Next topic will explain why distance matters."
        state.dirty = True
        return True
    ...
    if event.kind == EventKind.ENC2_DELTA:
        ...
        state.status = f"Trail length: {state.max_iter}"
        state.dirty = True
        return True
    ...
    if event.kind == EventKind.MENU_TOGGLE:
        state.status = "Menu comes later; this view is focused on the orbit."
        state.dirty = True
        return True

    if event.kind == EventKind.FUNCTION:
        state.status = "This button is unused for this view."
        state.dirty = True
        return True
...
def handle_event(state: Scene2State, event: Event, now: float, fade_s: float) -> bool:
...
def update_scene(state: Scene2State, now: float) -> None:
    ...
    if state.current_iter < end_iter:
        ...
        state.playing = False
        state.status = "Reached the displayed trail length."
        state.dirty = True
...
class Scene2Renderer:
    ...
    def intro_animating(self, state: Scene2State, now: float) -> bool:
        return False

    def draw(self, state: Scene2State, now: float) -> np.ndarray:
        if state.phase == AppPhase.INTRO:
    ...
    def _draw_intro(self, state: Scene2State, now: float) -> Image.Image:
    ...
    def _draw_interactive(self, state: Scene2State) -> Image.Image:
        img = Image.new("RGB", (self.width, self.height), self.bg)
        draw = ImageDraw.Draw(img)
        palette = PALETTES[state.palette_index]
        accent = palette["accent"]
    ...
    def _draw_interactive_title(self, draw, state: Scene2State, accent) -> None:
        self._text(draw, (58, 28), "A rule in the plane", self.font_large, self.white)
        self._text(draw, (60, 70), "z → z² + c", self.font_formula, accent)
        ...
        self._text(draw, (520, 92), f"trail length: {state.max_iter}    view: ±{state.view_radius:.2f}", self.font_mono_small, self.dim)

    def _draw_complex_plane(self, draw, state: Scene2State, palette) -> None:
        box = (62, 145, 1218, 610)
        self._panel(draw, box, self.panel, self.panel_outline, width=2, radius=24)
        x0, y0, x1, y1 = box

        self._text(draw, (x0 + 28, y0 + 10), "Complex plane", self.font_medium, palette["accent"])
        ...
        for ring in [1.0, 2.0]:
            if ring <= radius:
                ...
                draw.ellipse((origin[0] - r_px, origin[1] - r_px, origin[0] + r_px, origin[1] + r_px), outline=(42, 51, 64), width=1)
        ...
        self._draw_side_readout(draw, state, side_x, y0 + 76, x1 - 42, y1 - 56, values, diverged, palette)
    ...
    def _draw_side_readout(self, draw, state: Scene2State, x0: int, y0: int, x1: int, y1: int,
                           values: Sequence[complex], diverged: bool, palette) -> None:
        draw.rounded_rectangle((x0, y0, x1, y1), radius=18, fill=(11, 15, 22), outline=(39, 52, 62), width=1)

        z = values[-1]
        self._text(draw, (x0 + 22, y0 + 18), "Current orbit", self.font_medium, palette["accent"])
        self._text(draw, (x0 + 22, y0 + 64), f"n = {state.current_iter}", self.font_mono_small, self.white)
        self._text(draw, (x0 + 22, y0 + 94), f"zₙ = {fmt_complex(z)}", self.font_mono_small, palette["current"])
        self._text(draw, (x0 + 22, y0 + 124), f"|zₙ| = {abs(z):.4f}", self.font_mono_small, self.white)

        self._text(draw, (x0 + 22, y0 + 176), "Chosen point", self.font_medium, palette["c_point"])
        self._text(draw, (x0 + 22, y0 + 222), f"c = {state.c_r:+.4f} {state.c_i:+.4f}i", self.font_mono_small, self.white)
        recent = values[-3:]
        recent_text = "  →  ".join(f"{abs(v):.2f}" if abs(v) < 1000 else f"{abs(v):.1e}" for v in recent)
        self._text(draw, (x0 + 22, y1 - 75), "recent |z|", self.font_small, self.dim)
        self._text(draw, (x0 + 22, y1 - 45), recent_text, self.font_mono_small, self.white)

        if diverged:
            self._text(draw, (x0 + 22, y1 - 112), "far from centre", self.font_small, palette["current"])

    def _draw_compact_controls(self, draw, state: Scene2State, accent) -> None:
        box = (62, 632, 1218, 694)
        self._panel(draw, box, (12, 16, 23), (45, 62, 70), width=1, radius=18)

        visible, diverged = visible_values(state)
        message = classify_orbit(visible, diverged)

        self._text(draw, (86, 642), message, self.font_normal, self.white)
        self._text(draw, (86, 670), state.status, self.font_small, self.dim)

        controls = "Joystick move c   B3 play/pause   Enc1 zoom   Enc2 trail   stick reset   B2 next   B1 back   B5 palette"
...
def run_scene(args: argparse.Namespace) -> None:
    ...
    renderer = Scene2Renderer(args.width, args.height, swap_rb=args.swap_rb, fade_s=args.fade_seconds)
    state = Scene2State(max_iter=args.iterations)
    state.intro_fade_start = time.monotonic()

    print()
    print(f"Complex-plane recurrence walkthrough is now running on HDMI. ({SCRIPT_VERSION})")
    print("Use keyboard over SSH as the controller emulator. Press Q to quit.")
    print("N continue | B back | Space reveal/play | WASD/arrows move c | [ ] zoom | - = trail | R reset")
...
def self_test() -> None:
    ...
    renderer = Scene2Renderer(1280, 720)
    state.phase = AppPhase.INTRO
    state.intro_fade_start = time.monotonic() - 1.0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="FractalScope complex-plane recurrence HDMI prototype")
    ...
    parser.add_argument("--iterations", type=int, default=18, help="Initial displayed iteration/trail length")
