...
module end_to_end_pipeline_tb;
    ...
    localparam int PROGRESS_EVERY = 250_000;

    // 0 = test the intended/correct integration with SOF/EOL aligned to palette RGB.
    // 1 = reproduce the block-design-looking direct ROB->packer SOF/EOL wiring.
    localparam bit USE_BLOCK_DESIGN_SOF_EOL = 1'b0;
    ...
    // SOF/EOL must travel with the registered palette output, not directly with the reorder-buffer output. 
    // These registers use exactly the same enable as colour_palette's output register.
    logic sof_aligned;
    logic eol_aligned;
    always_ff @(posedge clk) begin
        ...
        else if (palette_in_ready) begin
            sof_aligned <= rob_out_valid && rob_out_sof;
            eol_aligned <= rob_out_valid && rob_out_eol;
        end
    end

    wire packer_sof = USE_BLOCK_DESIGN_SOF_EOL ? rob_out_sof : sof_aligned;
    wire packer_eol = USE_BLOCK_DESIGN_SOF_EOL ? rob_out_eol : eol_aligned;
    ...
    // Deterministic backpressure on the AXI stream sink.
    logic [7:0] ready_lfsr;
    always_ff @(posedge clk) begin
        if (rst) begin
            ready_lfsr  <= 8'hA5;
            axis_tready <= 1'b0;
            ...
            axis_tready <= ready_lfsr[0] | ready_lfsr[3] | ready_lfsr[5];
        end
    end
    ...
    initial begin
        $display("================================================");
        $display(" end_to_end_pipeline_tb_1280x720 starting");
        ...
        $display(" USE_BLOCK_DESIGN_SOF_EOL=%0d", USE_BLOCK_DESIGN_SOF_EOL);
        $display("================================================");
        ...
        for (int cycle = 0; cycle < TIMEOUT_CYCLES; cycle++) begin
            ...
            if ((cycle != 0) && ((cycle % PROGRESS_EVERY) == 0)) begin
                $display("[PROGRESS] cycle=%0d dispatched=%0d/%0d rob=%0d palette=%0d axis_words=%0d/%0d fails=%0d",
            ...
            if ((palette_accepts == FRAME_PIXELS) && (axis_words == EXPECTED_AXIS_WORDS)) begin
                ...
                $display("================================================");
                $display(" End-to-end summary: dispatched=%0d rob_pixels=%0d palette_pixels=%0d axis_words=%0d fails=%0d", 
                         scheduler_dispatches, rob_accepts, palette_accepts, axis_words, fails);
                $display("================================================");

                if (fails == 0) begin
                    $display("[PASS] end_to_end_pipeline_tb_1280x720 completed successfully");
                    $finish;
                end
                else begin
                    $fatal(1, "[FAIL] end_to_end_pipeline_tb_1280x720 completed with %0d failure(s)", fails);
                end
            end
        end

        $display("================================================");
        $display("[TIMEOUT] dispatched=%0d rob_pixels=%0d palette_pixels=%0d axis_words=%0d fails=%0d", 
                 scheduler_dispatches, rob_accepts, palette_accepts, axis_words, fails);
        $display("================================================");
        $fatal(1, "end_to_end_pipeline_tb_1280x720 timed out");
    end
